...
export default function ProfilePage(){
  ...
  const { userbasic , areUserLoading } =  useUser();
  ...
  console.log(userbasic)
        ...
        
    {
        areUserLoading ? (
          ...
          <h1 className='text-gray-500'>Please wait while we build Profile for you...</h1>
          </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
            <div className="col-span-4 sm:col-span-3">
                <div className="bg-white shadow rounded-lg p-6">
                    <div className="flex flex-col items-center">
                        <img src={userbasic?.userDetails.profile} className="w-32 h-32 bg-gray-300 rounded-full mb-4 shrink-0">

                        </img>

                        <h1 className="text-xl font-bold">{userbasic?.userDetails.name}</h1>
                        {
                            userbasic?.userExperience.map((exp) => (
                                <p className="text-gray-600">{exp.title}</p>
                            ))
                        }
                        <div className="mt-6 flex flex-wrap gap-4 justify-center">
                          <Link href='/editprofile' > <a className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded">Edit Profile</a></Link>
                            <a href={userbasic?.userDetails.resume} className="bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 px-4 rounded">Resume</a>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <ul>
                            {
                                userbasic?.userSkills.map((skill) => (
                                    <li className="mb-2">{skill.skillName}</li>
                                ))
                            }
                        </ul>
                    </div>
            ...
            <div className="col-span-4 sm:col-span-9">
                <div className="bg-white shadow rounded-lg p-6">
                    ...
                    <p className="text-gray-700">{userbasic?.userDetails.about}
                    </p>
                    ...
                    <h2 className="text-xl font-bold mt-6 mb-4">Experience</h2>
                    {
                        ...
                        <div className="flex justify-between">
                            <span className="text-gray-600 font-bold">{exp.title}</span>
                            <p>
                                <span className="text-gray-600 mr-2">{exp.company}</span>
                                <span className="text-gray-600">{exp.duration}</span>
                            </p>
                        </div>
                        <p className="mt-2">{exp.summary}
                        </p>
                    </div>
                        ))
                    }
